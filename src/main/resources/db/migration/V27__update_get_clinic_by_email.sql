DROP FUNCTION IF EXISTS public.get_clinic_by_email(text);

CREATE FUNCTION public.get_clinic_by_email(p_email text)
 RETURNS TABLE(
    id uuid,
...
AS $function$
    SELECT id, name, tax_id, email, phone, address, city, country,
           ...
           registration_number, activity_code, bank_account, vat_payer,
           veterinary_license_number, phone_country_code
    FROM clinic
    WHERE email = p_email
      AND deleted = false
    LIMIT 1;
