-- ============================================================
-- VetClinic Row-Level Security Setup
-- Run once as superuser: psql -U postgres -d vetapp -f rls-setup.sql
-- ============================================================

-- 1. Create application-level DB user (non-superuser, RLS applies)
DO $$
BEGIN
...
-- NULLIF is required for PostgreSQL 18+ where current_setting with missing_ok=true
-- returns empty string '' instead of NULL. Without NULLIF, ''::uuid throws an error.
-- With NULLIF: unset variable -> '' -> NULL -> clinic_id = NULL -> false -> no rows (fail-safe)
...
-- Used by refresh token flow (no clinic context yet) and DataSeeder startup check

-- Get clinic_id for a user by their ID (used in refresh token flow)
CREATE OR REPLACE FUNCTION get_clinic_id_for_user(p_user_id UUID)
RETURNS UUID AS $$
    SELECT clinic_id FROM users WHERE id = p_user_id;
$$ LANGUAGE SQL SECURITY DEFINER STABLE;

-- Count all users across all clinics (used by DataSeeder to check if DB needs seeding)
...
-- 5. Verification query (run as vetapp_user to test)
-- SET app.current_clinic_id = '<your-clinic-uuid>';
-- SELECT * FROM users; -- should only return rows for that clinic
-- RESET app.current_clinic_id;
-- SELECT * FROM users; -- should return 0 rows (fail-safe)
